        ...
        <b-row>
          ...
          <b-col v-if="isPaymentMethod == 2" md="6">
            <b-form-group label="Banks">
              <select v-model="form.bank_account_id" class="form-control custom-input-control" @change="checkBalance">
          ...
          <b-col v-if="isPaymentMethod == 3" md="6">
            <b-form-group label="Mobile Bank">
              <select v-model="form.mobile_bank_account_id" class="form-control custom-input-control" @change="checkBalance">
                <option v-for="(account, index) in accounts" :value="account.id" :key="index">
                  {{ account.name }} - {{ account.account_no }} - {{ amountFormat(account.amount) }}
                </option>
              </select>
...
export default {
  ...
  methods: {
    ...
    checkBalance(event) {
      ...
      let bankAmount = text.split('-')[2];
      let cashAmount = this.form.cash_out;

      if (!cashAmount) {
        alert(`Please enter you amount first`);
        this.form.bank_account_id = '';
        this.form.mobile_bank_account_id = '';
      }

      bankAmount = Number(bankAmount.split(',').join(''));

      if (cashAmount > bankAmount) {
        alert(`Sorry! You don't have enough balance to expance`);
        this.form.cash_out = '';
        this.form.bank_account_id = '';
        this.form.mobile_bank_account_id = '';
      }
